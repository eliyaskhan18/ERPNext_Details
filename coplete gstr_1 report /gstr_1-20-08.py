...
class Gstr1Report(object):
	...
	def run(self):
		...
		if self.invoices:
			...
			for item_list in self.data:
				...
				self.invoice_ID = item_list[1]
				self.customer_address_bill = item_list[2]
				advance_payment_detais = self.get_Advance_Payment_details()
				#print "customer_address-----------",self.customer_address_bill
				self.docstatus = item_list[44]
				self.amended_from = item_list[45]
				self.invoice_type = item_list[46]
				#print "self.amended_from------------",self.amended_from
				...
				for items in invoice_items_data:
					...
					if len(advance_payment_detais)!=0:
						...
						#print "voucher_number--------",voucher_number
						#print "date---------",advance_reciept_date
						...
						if self.invoice_type == "Export":
							for export_items in export_Details:
								...
								if str(self.invoice_ID) == str(invoice_id):
									...
									#print "shipping_bill_number-----------",shipping_bill_number
									#print "shipping_bill_date-----------",shipping_bill_date
						else:
							export_type = "--"
							port_code = "--"
							shipping_bill_number = "--"
					...
					for invoice_value in self.value_of_invoice_data:			
						...
						if self.invoice_ID in sales_invoice_id:
							...
							if self.amended_from is not None:
								...
								#print "original_invoice_id-------",original_invoice_id
							else:
								original_invoice_date = "--" 
								original_invoice_id = "--"
								original_customer_gstin = "--"				
							...
							dealer_registered = invoice_value.composition_dealer_or_uin_registered
							
							if dealer_registered == 1:
								check_for_registed = "Yes"
								#print "check_for_registed-------",check_for_registed
							else:
					...
					#print "tax_data_details------",tax_data_details	
					...
					item_table_details = self.get_Item_details()
					#print "item_table_details--------",item_table_details
					item_good_service=""
					for item_names in item_table_details:
						name_of_item = item_names.item_name
						#print "name_of_item--------",name_of_item
						if self.item_name == name_of_item:
							good_or_service = item_names.is_the_item_is_good_or_serivce
							if good_or_service != None:
								if "S" in good_or_service:
									item_good_service = good_or_service
								elif "G" in good_or_service:
									item_good_service = good_or_service
							else :
								item_good_service ="-"
					Address_details = self.get_contact_details()
					city = ""
					state =""
					if self.customer_address is not None:
						for address in Address_details:
							...
							if Address_name == self.customer_address:
								city = address.city
						       		state = address.state
					else:
						city = "--"
					...
					item_list[4], item_good_service, self.item_name, gst_hsn_code, qty, uom, rate, total_disc, 						price_list_rate, cgst_tax_rate ,cgst_tax_amount,sgst_tax_rate,sgst_tax_amount,
					igst_tax_rate,igst_tax_amount,cess_tax_rate,cess_tax_amount,check_bill_of_supply,
					nil_rate_non_gst_item,item_list[23],export_type,port_code,shipping_bill_number,
					shipping_bill_date,has_gst_or_idt,item_list[29], item_list[30],city,state,
					is_cancelled,check_for_registed,Return_Filing_Quarter,original_invoice_date,
					original_invoice_id,original_customer_gstin,item_list[39],advance_reciept_date,
					voucher_number,advance_amount, item_list[43], 
					item_list[44], item_list[45], item_list[46]])
	...
	def get_invoice_data(self):
		...
		invoice_data = frappe.db.sql("""
			select
				...
				composition_dealer_or_uin_registered,posting_date,docstatus,shipping_bill_number,
				shipping_bill_date,port_code,export_type
	...
	def get_invoice_items_details(self): 
		Sales_invoice_item = frappe.db.sql("""
			select
				parent, (price_list_rate-rate) as total_disc,item_name,
				gst_hsn_code,qty,uom,rate,price_list_rate,nil_rated_or_exempt_or_nongst_item
			from `tabSales Invoice Item`
			where
				parent in (%s)
		""",(self.invoice_ID), as_dict=1)
	...
	def get_items_based_on_tax_rate(self):
		...
		#print "self.details_tax---------------------",self.details_tax
	...
	def get_tax_details(self):
		...
		for tax_list in self.tax_details:
			...
			#print "account_head------",self.account_head
			cgst_rate = 0
			cgst_amount = 0
			sgst_rate = 0
			sgst_amount = 0
			...
			if tax_invoice_id == self.invoice_ID:
				if self.item_name in convert_dict:
					tax_rate = convert_dict.get(self.item_name)
					#print "account_head-------",account_head
					
					if "CGST" in account_head:
						#print "CGST items cgst_rate----------",self.item_name
						cgst_rate = tax_rate[0]
						cgst_amount = tax_rate[1]
						#print "tax_rate----------",cgst_rate
						#print "tax_amount----------",cgst_amount
						...
						#print "SGST items cgst_rate----------",self.item_name
						sgst_rate = tax_rate[0]
						sgst_amount = tax_rate[1]
						#print "tax_rate----------",sgst_rate
						#print "tax_amount----------",sgst_amount
						tax_data_json['sgst_rate'] = sgst_rate
						tax_data_json['sgst_amount'] = sgst_amount
						tax_data.append(tax_data_json)
					elif "IGST" in account_head:
						#print "IGST items cgst_rate----------",self.item_name
						igst_rate = tax_rate[0]
						igst_amount = tax_rate[1]
						#print "tax_rate----------",igst_rate
						#print "tax_amount----------",igst_amount
	...
	def get_contact_details(self):
		contact_details = frappe.db.sql("""
			...
			where
				name = %s
		""",(self.customer_address), as_dict=1)
		#print "contact_details----------",contact_details
		return contact_details
	def get_Advance_Payment_details(self):
		Advance_details = frappe.db.sql("""
			select
				paid_amount,name,creation,party_name
			from `tabPayment Entry`
			where
				party_name = %s
		""",(self.customer_address_bill), as_dict=1)
		#print "Advance_details----------",Advance_details
		return Advance_details

			
	def get_Item_details(self):
		Details_of_Item = frappe.db.sql("""
			select
				item_name,is_the_item_is_good_or_serivce
			from `tabItem`
			where
				item_name = %s
		""",(self.item_name), as_dict=1)
		return Details_of_Item
