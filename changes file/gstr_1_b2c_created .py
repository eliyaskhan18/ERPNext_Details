...
class Gstr1Report(object):
				...
				for items in invoice_items_data:
					...
					for items_data in tax_data_details:
						...
						if "cess_amount" in items_data:
							...
							cess_tax_amount = "--"
					item_good_service =""
					item_name,is_the_item_is_good_or_serivce = self.get_Item_details()
					if item_name:
					...
					city = ""
					state =""
					name,city,state = self.get_contact_details()
					if name:
					...
					if self.filters.get("type_of_business") ==  "B2C Large":
						if grand_total > float(b2c_limit) and address_details != self.gst_state_number:
							summ_data.append([item_list[0], item_list[1], item_list[2], item_list[3],
							item_list[4], item_good_service, self.item_name, gst_hsn_code, qty, uom, rate,
							total_disc,price_list_rate,cgst_tax_rate ,cgst_tax_amount,sgst_tax_rate,
							sgst_tax_amount,igst_tax_rate,igst_tax_amount,cess_tax_rate,
							cess_tax_amount,check_bill_of_supply,nil_rate_non_gst_item,item_list[23],
							export_type,port_code,shipping_bill_number,shipping_bill_date,has_gst_or_idt,
							item_list[29], item_list[30],city,state,is_cancelled,check_for_registed,
							Return_Filing_Quarter,original_invoice_date,original_invoice_id,
							original_customer_gstin,item_list[39],advance_reciept_date,
						...
						if grand_total <= float(b2c_limit) and address_details != self.gst_state_number:
							...
							item_list[44], item_list[45], item_list[46]])
							#summ_data.append(["","","","","",""])
						elif grand_total <= float(b2c_limit) and address_details == self.gst_state_number:
							summ_data.append([item_list[0], item_list[1], item_list[2], item_list[3],
							item_list[4], item_good_service, self.item_name, gst_hsn_code, qty, uom, rate,
							total_disc,price_list_rate,cgst_tax_rate ,cgst_tax_amount,sgst_tax_rate,
							sgst_tax_amount,igst_tax_rate,igst_tax_amount,cess_tax_rate,
	...
	def get_invoice_data(self):
		...
		invoice_data = frappe.db.sql("""
			select
				...
				shipping_bill_date,port_code,export_type,grand_total
			from `tab{doctype}`
	...
	def get_contact_details(self):
		...
		if self.customer_address:
			name,city,state,self.gst_state_number = frappe.db.get_value('Address',self.customer_address,
	...
	def get_Item_details(self):
		...
		if self.item_name:
			item_name,is_the_item_is_good_or_serivce = frappe.db.get_value('Item',self.item_name,
			['item_name','is_the_item_is_good_or_serivce'])
		return item_name,is_the_item_is_good_or_serivce

	def address_gst_number(self):
		#print "hello address"
		address_detail = frappe.get_list("Address", ["address_type","gst_state_number"])
		for itrate_address in address_detail:
			address_type = itrate_address.address_type
			if str(address_type) == "Billing":
				self.gst_bill_state_number = itrate_address.gst_state_number
		return self.gst_bill_state_number
